...
function hash_meta_callback($post) {
    ...
    <div>
        <div class="meta-row">
            ...
            <div class="meta-td">
                <input type="text" name="news_id" id="news_id" value="<?php if ( !empty($hash_stored_meta['news_id'])) { echo esc_attr( $hash_stored_meta['news_id'][0]); } ?>">
            </div>
        </div>
    </div>
    <div>
        <div class="meta-row">
            ...
            <div class="meta-td">
                <input type="textarea" name="news_title" id="news_title" value="<?php if ( !empty($hash_stored_meta['news_title'])) { echo esc_attr( $hash_stored_meta['news_title'][0]); } ?>">
            </div>
        </div>
    </div>
    <div>
    ...
    <div class="meta">
        <div class="meta-th">
            <span><?php _e('News Body', ''); ?></span>
        </div>
    </div>
    <div class="meta-editor">
        <?php
        $content = get_post_meta($post->ID, 'news_body', true);
        $editor = 'news_body';
        $settings = array(
            'textarea_rows' => 8,
            'media_buttons' => true
...
function hash_meta_save($post_id) {
    ...
    $is_valid_nonce = (isset($_POST['hash_newss_nonce']) && wp_verify_nonce($_POST['hash_newss_nonce'], basename(__FILE__))) ? 'true' : 'false';
    ...
    if (isset($_POST['news_id'] )) {
        update_post_meta($post_id, 'news_id', sanitize_text_field($_POST['news_id']));
    }
    if (isset($_POST['news_title'] )) {
        update_post_meta($post_id, 'news_title', sanitize_text_field($_POST['news_title']));
    }
